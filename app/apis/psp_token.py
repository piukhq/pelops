...
def check_token(action_code: str, psp_token: str) -> tuple:
    ...
    The short form of the psp_token is:
        ERR{code}_#1
        where {code} is a string such as ADD see codes below and #1 is the returned payment error string if applicable

    The long form of the psp_token is:
        ...
        #5 = Unique part of token - must be changed on each request using same code to ensure a reset of times counter
    ...
    Examples where RET code is used to define an error during retain:
        ...
        Fail retain next 4 times requests will return normal error after 0 seconds and on 5th and subsequent pass
         normally (sepcial case with retain is that there is no pay _error so xxx will never be used) - with this
        ...
        Fail retain next 2 times returning 504 after 20 seconds and on 5th and subsequent pass normally:
